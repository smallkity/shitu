...
export function PhotoCard({ photo, tags, onOpen, onDelete }: PhotoCardProps) {
  return (
    <article className="photo-card">
      <button className="photo-card-image" onClick={onOpen} aria-label={`查看 ${photo.fileName}`}>
      ...
      <div className="photo-card-content">
        <div className="photo-card-title-row">
          ...
          <button className="icon-button subtle" onClick={onOpen} aria-label="编辑标签">
            <Edit3 size={16} />
          </button>
        </div>
        <div className="photo-card-tags">
          {tags.slice(0, 3).map((tag) => <span key={tag.id} className="mini-tag">{tag.name}</span>)}
          {tags.length > 3 && <span className="mini-tag more">+{tags.length - 3}</span>}
        </div>
